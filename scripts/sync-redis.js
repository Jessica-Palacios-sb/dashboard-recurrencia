...

  {
    ...
    async run(client) {
      const [r1, r2] = await Promise.all([
        client.query(`
WITH e AS (
  SELECT c.student_id, CAST(c.fecha_cierre AS date) AS fecha_cierre, CAST(c.fecha_cancelacion AS date) AS fecha_cancelacion, c.tipo_cancelacion, c.canal_cancelacion,
...
SELECT TO_CHAR(DATE_TRUNC('month', fecha_cancelacion),'YYYY-MM') AS mes_cancelacion, TO_CHAR(DATE_TRUNC('month', fecha_cierre),'YYYY-MM') AS mes_inicio,
  ...
  tipo_pago, tipo_cliente, pais_agrupado, DATEDIFF('month', fecha_cierre, fecha_cancelacion) AS meses_vida_real,
  COUNT(*) AS suscripciones, AVG(DATEDIFF('month', fecha_cierre, fecha_cancelacion)) AS avg_meses_activo
FROM e WHERE DATEDIFF('month', fecha_cierre, fecha_cancelacion) >= 0
GROUP BY 1,2,3,4,5,6,7 ORDER BY mes_cancelacion, mes_inicio;`),
        client.query(`
SELECT TO_CHAR(DATE_TRUNC('month', c.fecha_cierre),'YYYY-MM') AS mes,
  CASE WHEN est.pais_agrupado IN ('México','Mexico') THEN 'México' WHEN est.pais_agrupado = 'Colombia' THEN 'Colombia' WHEN est.pais_agrupado IN ('Estados Unidos','United States') THEN 'Estados Unidos' ELSE 'Otros' END AS pais_agrupado,
  CASE WHEN LOWER(COALESCE(c.tipo_cliente,'')) LIKE '%cuotas%' THEN 'Cuotas' ELSE 'Recurrencia' END AS tipo_pago,
  COALESCE(NULLIF(TRIM(c.tipo_cliente),''),'(sin)') AS tipo_cliente, COUNT(*) AS nuevos_clientes
FROM salesforce.tabla_intermedia_estado_clientes c LEFT JOIN salesforce.tabla_core_estudiantes est ON c.student_id = est.student_id
WHERE c.tipo_oportunidad = 'Suscripciones' AND c.fecha_cierre >= '2023-08-01'
GROUP BY 1,2,3,4 ORDER BY mes;`),
      ]);
  ...
  SELECT student_id, CAST(fecha_cierre AS date) AS fecha_cierre, estado_usuario, sub_estado_usuario,
    CAST(fecha_cancelacion AS date) AS fecha_cancelacion, tipo_cancelacion, canal_cancelacion
  FROM salesforce.tabla_intermedia_estado_clientes WHERE tipo_oportunidad = 'Suscripciones' AND fecha_cierre >= '2023-08-01'
)`;
      const QUERY_FLUJO = `
WITH e AS (SELECT c.student_id, CAST(c.fecha_cierre AS date) AS fecha_cierre, c.estado_usuario, CAST(c.fecha_cancelacion AS date) AS fecha_cancelacion, c.tipo_cancelacion, c.canal_cancelacion,
    COALESCE(NULLIF(TRIM(c.tipo_cliente),''),'(sin)') AS tipo_cliente,
  ...
  FROM salesforce.tabla_intermedia_estado_clientes c LEFT JOIN salesforce.tabla_core_estudiantes est ON c.student_id = est.student_id
  WHERE c.tipo_oportunidad = 'Suscripciones' AND c.fecha_cierre >= '2023-08-01'),
nuevos AS (SELECT TO_CHAR(DATE_TRUNC('month', fecha_cierre),'YYYY-MM') AS mes, pais, tipo_cliente, COUNT(*) AS nuevos FROM e WHERE fecha_cierre IS NOT NULL GROUP BY 1,2,3),
cancel AS (SELECT TO_CHAR(DATE_TRUNC('month', fecha_cancelacion),'YYYY-MM') AS mes, pais, tipo_cliente, COUNT(*) AS cancelados,
...
WHERE h.fecha_susp IS NOT NULL GROUP BY 1, 2, 3 ORDER BY 1;`;
      const QUERY_PAIS_EC = `
WITH e AS (SELECT c.student_id, c.estado_usuario, c.fecha_cancelacion, c.tipo_cancelacion,
    CASE WHEN est.pais_agrupado IN ('México','Mexico') THEN 'México' WHEN est.pais_agrupado = 'Colombia' THEN 'Colombia' WHEN est.pais_agrupado IN ('Estados Unidos','United States') THEN 'Estados Unidos' ELSE 'Otros' END AS pais
  FROM salesforce.tabla_intermedia_estado_clientes c LEFT JOIN salesforce.tabla_core_estudiantes est ON c.student_id = est.student_id
  WHERE c.tipo_oportunidad = 'Suscripciones' AND c.fecha_cierre >= '2023-08-01'),
...
SELECT b.pais AS pais_agrupado, b.clientes_totales, COALESCE(c.cancelaciones,0) AS cancelaciones, COALESCE(c.por_mora,0) AS por_mora, COALESCE(c.voluntaria,0) AS voluntaria,
  ROUND(COALESCE(c.cancelaciones,0) * 100.0 / NULLIF(b.clientes_totales,0), 2) AS tasa_churn_pct
FROM base b LEFT JOIN cancel c ON b.pais = c.pais ORDER BY cancelaciones DESC;`;
      const QUERY_VIDA_EC = `
WITH e AS (SELECT c.student_id, CAST(c.fecha_cierre AS date) AS fecha_cierre, CAST(c.fecha_cancelacion AS date) AS fecha_cancelacion, c.tipo_cancelacion, c.canal_cancelacion, c.estado_usuario,
    COALESCE(NULLIF(TRIM(c.tipo_cliente),''),'(sin)') AS tipo_cliente,
    CASE WHEN est.pais_agrupado IN ('México','Mexico') THEN 'México' WHEN est.pais_agrupado = 'Colombia' THEN 'Colombia' WHEN est.pais_agrupado IN ('Estados Unidos','United States') THEN 'Estados Unidos' ELSE 'Otros' END AS pais
  FROM salesforce.tabla_intermedia_estado_clientes c LEFT JOIN salesforce.tabla_core_estudiantes est ON c.student_id = est.student_id
  WHERE c.tipo_oportunidad = 'Suscripciones' AND c.fecha_cierre >= '2023-08-01'),
...
FROM e GROUP BY 1, 2, 3, 4;`),
        ...
        (async()=>{ const c3=getClient(); await c3.connect(); try { return await c3.query(QUERY_SUSP); } catch(e){ console.error('susp:', e.message); return {rows:[]}; } finally { await c3.end(); } })(),
      ]);
      await c2.end();
      return { nuevos: r1.rows, cancelaciones: r2.rows, tasaChurn: r3.rows, tiempoVida: r6.rows, motivos: r4.rows, churnPais: r5.rows, flujo: r7.rows, suspendidos: r8.rows };
    },
  },
...
pagos_semana AS (
  SELECT 'psem' AS tipo, pais, tipo_cliente, CASE WHEN tipo_pago = 'Cuotas' THEN 'Cuotas' ELSE 'Recurrencia' END AS tipo_pago, tipo_venta,
    ...
    SUM(CASE WHEN payment_amount_usd > 0 THEN 1 ELSE 0 END) AS n,
    ROUND(SUM(COALESCE(payment_amount_usd, 0))::numeric, 0) AS cash
  FROM detalle WHERE fecha_cierre IS NOT NULL AND fecha_pago IS NOT NULL AND payment_amount_usd > 0 GROUP BY 2,3,4,5,6,7
)
